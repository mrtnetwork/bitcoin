...
func TestP2WSH(t *testing.T) {
	...
	sk1 := keypair.NewECPrivateFromWIF("cTALNpTpRbbxTCJ2A5Vq88UxT44w1PE2cYqiB3n4hRvzyCev1Wwo")
	sk2 := keypair.NewECPrivateFromWIF("cRvyLwCPLU88jsyj94L7iJjQX5C2f8koG4G2gevN4BeSGcEvfKe9")
	p2wshScript := scripts.Script(scripts.Script{Script: []interface{}{
		"OP_2", sk1.GetPublic().ToHex(true),
		sk2.GetPublic().ToHex(true),
		"OP_2",
		"OP_CHECKMULTISIG",
	}})
	p2wshAddr := address.P2WSHAddresssFromScript(p2wshScript)
	...
	t.Run("spend_from_p2wsh_multisig_to_p2pkh", func(t *testing.T) {
		...
		witness := scripts.TxWitnessInput{Stack: []string{"", sig1, sig2, pk}}
	...
	t.Run("spend_multiple_inputs_with_multiple_outputs", func(t *testing.T) {
		...
		witnesss1 := scripts.TxWitnessInput{Stack: []string{}}
		...
		witnesss2 := scripts.TxWitnessInput{Stack: []string{"", sigP2sh1, sigP2sh2, p2wshRedeemScript.ToHex()}}
		...
		witnesss3 := scripts.TxWitnessInput{Stack: []string{sig3, sk1.GetPublic().ToHex(true)}}
		// add witness to transaction
