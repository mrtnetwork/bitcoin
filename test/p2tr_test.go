...
func TestCreateP2trWithSingleTapScript(t *testing.T) {
	toPriv1 := keypair.NewECPrivateFromWIF("cT33CWKwcV8afBs5NYzeSzeSoGETtAB8izjDjMEuGqyqPoF7fbQR")
	toPub1 := toPriv1.GetPublic()
	privkeyTrScript1 := keypair.NewECPrivateFromWIF("cSW2kQbqC9zkqagw8oTYKFTozKuZ214zd6CMTDs4V32cMfH3dgKa")
	pubkeyTrScript1 := privkeyTrScript1.GetPublic()
	trScriptP2pk1 := scripts.Script{Script: []interface{}{
		pubkeyTrScript1.ToXOnlyHex(),
		"OP_CHECKSIG",
	}}
	toPriv2 := keypair.NewECPrivateFromWIF(
	...
	fromPriv2 := keypair.NewECPrivateFromWIF(
	...
	t.Run("spend_key_path2", func(t *testing.T) {
		tx := scripts.NewBtcTransaction([]scripts.TxInput{*txIn2},
			...
			true)
		digest := tx.GetTransactionTaprootDigest(0, []scripts.Script{scriptPubKey2}, []*big.Int{big.NewInt(3500)}, 0, scripts.Script{}, constant.LEAF_VERSION_TAPSCRIPT, constant.TAPROOT_SIGHASH_ALL)
		signatur := fromPriv2.SignTaprootTransaction(digest, constant.TAPROOT_SIGHASH_ALL, []interface{}{trScriptP2pk1}, true)
		witness := scripts.TxWitnessInput{Stack: []string{signatur}}
	...
	t.Run("spend_script_path2", func(t *testing.T) {
		tx := scripts.NewBtcTransaction([]scripts.TxInput{*txIn2},
			[]scripts.TxOutput{*txOut2},
			true)
		digest := tx.GetTransactionTaprootDigest(0, []scripts.Script{scriptPubKey2}, []*big.Int{big.NewInt(3500)}, 1, trScriptP2pk1, constant.LEAF_VERSION_TAPSCRIPT, constant.TAPROOT_SIGHASH_ALL)
		...
		witness := scripts.TxWitnessInput{Stack: []string{signatur, trScriptP2pk1.ToHex(), controllBlock.ToHex()}}
		tx.Witnesses = append(tx.Witnesses, witness)
		digestHex := tx.Serialize()
...
func TestCreateP2trWithTwoTapScripts(t *testing.T) {
	privkeyTrScriptA := keypair.NewECPrivateFromWIF("cSW2kQbqC9zkqagw8oTYKFTozKuZ214zd6CMTDs4V32cMfH3dgKa")
	pubkeyTrScriptA := privkeyTrScriptA.GetPublic()
	trScriptP2pkA := scripts.NewScript(pubkeyTrScriptA.ToXOnlyHex(), "OP_CHECKSIG")

	privkeyTrScriptB := keypair.NewECPrivateFromWIF("cSv48xapaqy7fPs8VvoSnxNBNA2jpjcuURRqUENu3WVq6Eh4U3JU")
	pubkeyTrScriptB := privkeyTrScriptB.GetPublic()
	trScriptP2pkB := scripts.NewScript(pubkeyTrScriptB.ToXOnlyHex(), "OP_CHECKSIG")

	fromPriv := keypair.NewECPrivateFromWIF("cT33CWKwcV8afBs5NYzeSzeSoGETtAB8izjDjMEuGqyqPoF7fbQR")
	...
	toPriv := keypair.NewECPrivateFromWIF("cNxX8M7XU8VNa5ofd8yk1eiZxaxNrQQyb7xNpwAmsrzEhcVwtCjs")
	...
	t.Run("test_spend_script_path_A_from_AB", func(t *testing.T) {
		...
		txDigest := tx.GetTransactionTaprootDigest(0, allUtxosScriptpubkeys, []*big.Int{big.NewInt(3500)}, 1, *trScriptP2pkA, constant.LEAF_VERSION_TAPSCRIPT, constant.TAPROOT_SIGHASH_ALL)
		...
		witness := scripts.TxWitnessInput{Stack: []string{signature, trScriptP2pkA.ToHex(), controlBlock.ToHex()}}
		tx.Witnesses = append(tx.Witnesses, witness)
...
func TestCreateP2trWithThreeTapScripts(t *testing.T) {
	...
	privkeyTrScriptC := keypair.NewECPrivateFromWIF("cRkZPNnn3jdr64o3PDxNHG68eowDfuCdcyL6nVL4n3czvunuvryC")
	...
	t.Run("test_spend_script_path_A_from_AB", func(t *testing.T) {
		tx := scripts.NewBtcTransaction([]scripts.TxInput{*txIn}, []scripts.TxOutput{*txOut}, true)
		txDigest := tx.GetTransactionTaprootDigest(0, allUtxosScriptpubkeys, []*big.Int{big.NewInt(3500)}, 1, *trScriptP2pkB, constant.LEAF_VERSION_TAPSCRIPT, constant.TAPROOT_SIGHASH_ALL)
		...
		witness := scripts.TxWitnessInput{Stack: []string{signature, trScriptP2pkB.ToHex(), controlBlock.ToHex()}}
		tx.Witnesses = append(tx.Witnesses, witness)
		if !strings.EqualFold(tx.Serialize(), signedTx3) {
			t.Errorf("Expected %v, but got %v", signedTx3, tx.Serialize())
		}
