...
func GenerateBitcoinPublicKey(privateKeyBytes []byte) ([]byte, error) {
	curve := P256k1() // Use the secp256k1 curve for Bitcoin
	x, y := curve.ScalarBaseMult(privateKeyBytes)
	publicKey := MarshalCompressed(curve, x, y)
...
func ReEncodedForm(public []byte, compressed bool) []byte {
	curve := P256k1() // Use the secp256k1 curve for Bitcoin
	x, y := UnmarshalCompressed(curve, public)
	if compressed {
...
func UnCompressedPoint(public []byte) (*big.Int, *big.Int) {
	if public[0] == 0x04 {
		return elliptic.Unmarshal(P256k1(), public)
	} else {
		return UnmarshalCompressed(P256k1(), public)
	}
}
...
func ReEncodedFromForm(public []byte, compressed bool) []byte {
	...
	toBytes := elliptic.Marshal(curve, x, y)[1:]
	return toBytes
}
func ScalarBaseMultBigInt(curve elliptic.Curve, k big.Int) (*big.Int, *big.Int) {
	e := encodeBigInt(&k)
	return curve.ScalarMult(curve.Params().Gx, curve.Params().Gy, e)
}
