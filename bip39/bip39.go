...
func EntropyToMnemonic(entropy []byte) string {
	...
	for i := 0; i < len(bits); i += 11 {
		...
		chunk := bits[i:end]
		index := formating.BinaryToByte(chunk)
		words = append(words, strings.Fields(wordlist)[index])
	}
...
func MnemonicToEntropy(mnemonic string) ([]byte, error) {
	...
	for i := 0; i < len(entropyBits); i += 8 {
		...
		byteValue := formating.BinaryToByte(binaryByte)
		entropyBytes = append(entropyBytes, byteValue)
	}
